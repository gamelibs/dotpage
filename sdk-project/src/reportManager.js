...
export const ReportManager = {
  ...
  userEvent: async function (eventType, data = {}) {
    if (CONFIG.LOCAL_MODE) {
      console.log('Local mode - Event:', eventType, data);
      return;
    }
  ...
  sendReport: function (url, data, retries = 1) {
    ...
    console.log("正在上报:", JSON.stringify(data, null, 2));
    ...
    const attempt = (currentRetries) => {
      fetch(url, {
        ...
        .then(result => {
          console.log('上报成功:', typeof result === 'string' ? result : JSON.stringify(result, null, 2));
        })
        .catch(error => {
          if (currentRetries > 0) {
